...
FROM 
    ...
    ON
        pivot_table.month = casual.month and 
        pivot_table.day_of_week = casual.day_of_week
    INNER JOIN 
    `ultra-sunset-341916.google_rideshare.pivot_member` AS member
    ON 
        pivot_table.month = member.month and 
        pivot_table.day_of_week = member.day_of_week
